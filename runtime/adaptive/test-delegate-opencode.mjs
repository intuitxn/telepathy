...
async function fixture(t,completed){
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  else if(route.endsWith('/message'))data=completed?[{info:{role:'assistant',time:{completed:Date.now()},finish:'stop',modelID:'existing',providerID:'configured',tokens:{input:7,output:2,reasoning:3,cache:{read:4}},cost:0.01},parts:[{type:'tool',tool:'bash',state:{status:'completed'}},{type:'text',text:'answer'}]}]:[];
