...
acceptance:
  ...
  - the bare-file fixture run exits 0, renders the header and five sorted rows, and prints `malformed_record_rejected: True` and `valid_fixture_well_formed: True`.
...
command_run() {
  ...
  if [ -f "$BEND" ]; then
    ...
    GATE_OUT=$( cd "$ROOT" && BEND_NO_TELEMETRY=1 "$BEND" "$OP_PATH" --check-only 2>&1 )
    GATE_CODE=$?
    FIXTURE_OUT=$( cd "$ROOT" && BEND_NO_TELEMETRY=1 "$BEND" "$OP_PATH" 2>&1 )
    FIXTURE_CODE=$?
  else
  ...
  GATE_CMD="BEND_NO_TELEMETRY=1 $BEND $OP_DISPLAY --check-only"
  FIXTURE_CMD="BEND_NO_TELEMETRY=1 $BEND $OP_DISPLAY"

  if [ "$GATE_CODE" -eq 0 ] && printf '%s' "$GATE_OUT" | grep -q 'All terms check\.'; then
    GATE_VERDICT=$GREEN; GATE_REASON="All terms check."
  elif printf '%s' "$GATE_OUT" | grep -q 'TODO'; then
    GATE_VERDICT=$YELLOW; GATE_REASON="open law / TODO present (informational)"
  ...
  if [ "$FIXTURE_CODE" -eq 127 ]; then
    ...
    fixture_lines=$(printf '%s\n' "$FIXTURE_OUT" | grep -c '[^[:space:]]')
    FIXTURE_VERDICT=$GREEN; FIXTURE_REASON="fixture exit 0 ($fixture_lines output lines)"
  fi
  ...
  if ! mkdir "$run_dir" 2>/dev/null; then
    printf 'could not create run dir: %s\n' "$run_dir" >&2
    return 1
  fi
  ...
  write_evidence
  {
    ...
    printf '%s\n' 'note: GREEN is not acceptance; only a named human reviewing the exact candidate revision resolves the work.'
  } > "$run_dir/verdict"
