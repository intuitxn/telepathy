...
`createAlgorithmTaskHost` in `task-host.mjs` runs one funded algorithm task in an exact-base jj workspace. Its SDK turn mounts the executable production patch without the research-only tool restriction, and the host keeps an immutable dispatch intent and response. It admits a candidate only when one executed `algorithm_run` receipt belongs to that session and its archived Bend bytes equal the sole changed file in the exact jj revision. Settlement uses the supplied branded `createJjCombinedVerifier`; that verifier reserves the evaluation, runs the exact release gate, and must score the receipt with a host-pinned oracle in `evaluateIndependent`. A passing source settlement still reports `task_accepted: false` until fresh-task comparison closes the whole goal. A missing response after a recorded dispatch requires host audit before another model turn.

The host can take `algorithmText` with its `model` declaration. For `model auto`, supply a private `modelIndexFile`, its pinned SHA-256, and `modelReceiptRoot`; the host checks each measured receipt against the index before selecting. Its dispatch intent binds the selected route. This release admits only routes backed by its pinned `deepseek-official/deepseek-flash` toolchain; other index entries cannot launch a provider turn. The index contains measurements and availability, never credentials.
