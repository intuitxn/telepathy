...
const guides = [
  ...
  'plugins/telepathy-meta-agents/bend-specialist.md',
  ...['telepathy', 'sansara', 'iktara'].map((name) => `programs/${name}.md`),
...
for (const guide of guides) {
  ...
  if (guide.startsWith('programs/')) {
    const retiredRouting = /\*\*agents:\*\*|\*\*runtime:\*\*\s*desk|@atlas|@forge|@ledger|@scout|@diplomat/gim;
    for (const match of source.matchAll(retiredRouting))
      errors.push(`${guide}:${line(match.index)}: retired agent routing in current program charter`);
  }
  ...
  for (const match of source.matchAll(/(?:^|`)\s*(?:\$\s*)?npm run ([a-z][a-z0-9:-]*)\b/gm)) {
    commands += 1;
    if (!(match[1] in scripts))
      errors.push(`${guide}:${line(match.index)}: missing root npm script ${match[1]}`);
  }
  for (const match of source.matchAll(/(?:^|`)\s*(?:\$\s*)?make ([a-z][a-z-]*)\b/gm)) {
    commands += 1;
